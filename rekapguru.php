...
<head>
    ...
    <title>Rekap Personal guru - Jurnal Mengajar Guru SMAN 1 Jepara</title>
...
<body>
    <!--body-->
    <div class="container">
        <div class="alert alert-info text-center">Halaman ini menampilkan data rekap personal Guru Per Semester.</div>
        <form action="" method="post">
            <select class="form-select" aria-label="nama Guru" name="namaguru">
                <option selected>Pilih Nama Guru</option>
                <?php while ($guru = $queryGuru->fetch_array()) { ?>
                    <option value="<?php echo $guru['KODEGURU'] ?>"><?php echo $guru['NAMA'] ?></option>

                <?php } ?>
            </select>
            <div class="d-grid gap-2">
                <button type="submit" name="viewrekap" class="btn btn-primary">Tampilkan Rekap</button>
            </div>
        </form>
        <p></p>
        <div class="">
            <?php
            if (isset($_POST['viewrekap'])) {
                $kodeguru = $_POST['namaguru'];
                $queryGuru = $con->query("SELECT KODEGURU,NAMA FROM guru WHERE KODEGURU='" . $kodeguru . "'");
                $data_guru = $queryGuru->fetch_array();
            ?>
                <p>Data Ditemukan:</p>
                <p>Nama Guru: <?php echo $data_guru['NAMA'] ?></p>
                <p>NIP: <?php echo $data_guru['KODEGURU'] ?></p>
                <div class="col-auto">
                    <a class="btn btn-primary" href="rekapguru_cetak.php?kg=<?php echo $data_guru['KODEGURU'] ?>" target="_blank"><i class="bi bi-printer"></i> CETAK REKAP PERSONAL</a>
                </div>
                <p></p>
                <table id="rekap" class="table table-striped nowrap" style="width:100%">
                    ...
                    <tbody>
                        ...
                        $stmts = $con->query("SELECT kg.TANGGAL, rk.NAMA_KELAS, kg.JAMKE, rm.NAMA_MAPEL,kg.HADIR, kg.KETERANGAN, kg.PBL_SELANJUTNYA, kg.SWAFOTO
											...
											WHERE kg.KODEGURU='" . $kodeguru . "' ORDER BY kg.TANGGAL ASC");
