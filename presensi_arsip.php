...
<body>
    <!--body-->
    <div class="container">
        <div class="alert alert-info text-center">Halaman ini memuat data arsip inputan bapak ibu guru.</div>
        <div class="">
            <div class="col-auto">
                <form action="" method="post">
                    <input type="hidden" name="guru" value="<?php echo $_SESSION['idnip'] ?>">
                    <select class="form-select" aria-label="nama Tahun" name="tahun">
                        <option selected>Pilih Tahun Ajaran</option>
                        <?php
                        $queryTp = $con->query("SELECT * FROM tahun_pelajaran WHERE AKTIF='yes'");
                        while ($atp = $queryTp->fetch_array()) { ?>
                            <option value="<?php echo $atp['IDTP'] ?>"><?php echo $atp['KETERANGAN'] ?></option>

                        <?php } ?>
                    </select>
                    <div class="d-grid gap-2">
                        <button type="submit" name="viewrekap" class="btn btn-primary">Tampilkan Arsip</button>
                    </div>
                </form>
                <?php
                if (isset($_POST['viewrekap'])) {
                    ...
                    $stmts = $con->query("SELECT kg.TANGGAL, rk.NAMA_KELAS, kg.JAMKE, rm.NAMA_MAPEL,kg.HADIR, kg.KETERANGAN, kg.PBL_SELANJUTNYA, kg.SWAFOTO
											...
											WHERE kg.KODEGURU='" . $_SESSION['idnip'] . "' and kg.IDTP='" . $tahun . "' ORDER BY kg.TANGGAL ASC");
                ?>
                    <hr />
                    <a class="btn btn-primary" href="rekapguru_cetak.php?kg=<?php echo $_SESSION['idnip']; ?>&tp=<?php echo $tahun; ?>" target="_blank"><i class="bi bi-printer"></i> CETAK REKAP PERSONAL</a>
